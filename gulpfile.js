...
var themeName         = 'clientsavvy', // Project name, used for build zip.
    siteRoot         = 'localhost:8888/new-wordpress/wordpress/', //used for browser-sync
    buildInclude    = [
...
gulp.task('watch-bs', ['browser-sync', 'sass', 'watch', 'scripts'], function () { });
...
// gulp dist-product
// Copies the files to the /dist folder for distributon
gulp.task('dist', ['clean-dist-product'], function() {
   // gulp.src(['**/*','!bower_components','!bower_components/**','!node_modules','!node_modules/**','!dist','!dist/**', '*'])
   gulp.src(buildInclude)
   .pipe(replace('Theme Name: Hexagon Sun', 'Theme Name: '+themeName))//update the stylesheet with the theme name for exporting theme
    .pipe(gulp.dest('../'+themeName+'/'))
});

// Deleting any file inside the /src folder
gulp.task('clean-dist-product', function () {
  return del(['../'+themeName+'/**/*',], { force: true });
});
